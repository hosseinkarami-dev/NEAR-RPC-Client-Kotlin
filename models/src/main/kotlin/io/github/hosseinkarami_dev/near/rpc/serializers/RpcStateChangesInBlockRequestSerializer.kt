      ...
      when (element) {
        ...
        is JsonObject -> {
          ...
          if (jobj.size == 1) {
            ...
            val typeField = jobj["type"]?.jsonPrimitive?.contentOrNull ?: throw SerializationException("Missing 'type' discriminator in RpcStateChangesInBlockRequest")
            when (typeField) {
              "block_id" -> {
              ...
              else -> throw SerializationException("Unknown type discriminator for RpcStateChangesInBlockRequest: " + typeField)
            }
          }
        }
      }
