...
public object InvalidAccessKeyErrorSerializer : KSerializer<InvalidAccessKeyError> {
  ...
  override fun serialize(encoder: Encoder, `value`: InvalidAccessKeyError) {
    ...
    when (value) {
      is InvalidAccessKeyError.AccessKeyNotFound -> out.encodeSerializableElement(descriptor, 0, serializer<InvalidAccessKeyError.AccessKeyNotFound.AccessKeyNotFoundPayload>(), value.accessKeyNotFound)
      is InvalidAccessKeyError.ReceiverMismatch -> out.encodeSerializableElement(descriptor, 1, serializer<InvalidAccessKeyError.ReceiverMismatch.ReceiverMismatchPayload>(), value.receiverMismatch)
      is InvalidAccessKeyError.MethodNameMismatch -> out.encodeSerializableElement(descriptor, 2, serializer<InvalidAccessKeyError.MethodNameMismatch.MethodNameMismatchPayload>(), value.methodNameMismatch)
      InvalidAccessKeyError.RequiresFullAccess -> out.encodeStringElement(descriptor, 3, "RequiresFullAccess")
      is InvalidAccessKeyError.NotEnoughAllowance -> out.encodeSerializableElement(descriptor, 4, serializer<InvalidAccessKeyError.NotEnoughAllowance.NotEnoughAllowancePayload>(), value.notEnoughAllowance)
      InvalidAccessKeyError.DepositWithFunctionCall -> out.encodeStringElement(descriptor, 5, "DepositWithFunctionCall")
    }
      ...
      when (element) {
        ...
        is JsonObject -> {
          ...
          if (jobj.size == 1) {
            ...
            val typeField = jobj["type"]?.jsonPrimitive?.contentOrNull ?: throw SerializationException("Missing 'type' discriminator in InvalidAccessKeyError")
            when (typeField) {
              "AccessKeyNotFound" -> {
              ...
              else -> throw SerializationException("Unknown type discriminator for InvalidAccessKeyError: " + typeField)
            }
          }
        }
      }
