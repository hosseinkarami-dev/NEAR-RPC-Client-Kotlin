...
public object StateChangeCauseViewSerializer : KSerializer<StateChangeCauseView> {
  ...
  override fun serialize(encoder: Encoder, `value`: StateChangeCauseView) {
    ...
    when (value) {
      is StateChangeCauseView.NotWritableToDisk -> out.encodeSerializableElement(descriptor, 0, serializer<StateChangeCauseView.NotWritableToDisk.Type>(), value.type)
      is StateChangeCauseView.InitialState -> out.encodeSerializableElement(descriptor, 1, serializer<StateChangeCauseView.InitialState.Type>(), value.type)
      is StateChangeCauseView.TransactionProcessing -> out.encodeSerializableElement(descriptor, 2, serializer<CryptoHash>(), value.txHash)
      is StateChangeCauseView.ActionReceiptProcessingStarted -> out.encodeSerializableElement(descriptor, 3, serializer<CryptoHash>(), value.receiptHash)
      is StateChangeCauseView.ActionReceiptGasReward -> out.encodeSerializableElement(descriptor, 4, serializer<CryptoHash>(), value.receiptHash)
      is StateChangeCauseView.ReceiptProcessing -> out.encodeSerializableElement(descriptor, 5, serializer<CryptoHash>(), value.receiptHash)
      is StateChangeCauseView.PostponedReceipt -> out.encodeSerializableElement(descriptor, 6, serializer<CryptoHash>(), value.receiptHash)
      is StateChangeCauseView.UpdatedDelayedReceipts -> out.encodeSerializableElement(descriptor, 7, serializer<StateChangeCauseView.UpdatedDelayedReceipts.Type>(), value.type)
      is StateChangeCauseView.ValidatorAccountsUpdate -> out.encodeSerializableElement(descriptor, 8, serializer<StateChangeCauseView.ValidatorAccountsUpdate.Type>(), value.type)
      is StateChangeCauseView.Migration -> out.encodeSerializableElement(descriptor, 9, serializer<StateChangeCauseView.Migration.Type>(), value.type)
      is StateChangeCauseView.BandwidthSchedulerStateUpdate -> out.encodeSerializableElement(descriptor, 10, serializer<StateChangeCauseView.BandwidthSchedulerStateUpdate.Type>(), value.type)
    }
      ...
      when (element) {
        ...
        is JsonObject -> {
          ...
          if (jobj.size == 1) {
            ...
            val typeField = jobj["type"]?.jsonPrimitive?.contentOrNull ?: throw SerializationException("Missing 'type' discriminator in StateChangeCauseView")
            when (typeField) {
              "not_writable_to_disk" -> {
              ...
              else -> throw SerializationException("Unknown type discriminator for StateChangeCauseView: " + typeField)
            }
          }
        }
      }
