...
public object RpcQueryResponseSerializer : KSerializer<RpcQueryResponse> {
  ...
  override fun serialize(encoder: Encoder, `value`: RpcQueryResponse) {
    ...
    when (value) {
      is RpcQueryResponse.AccountView -> out.encodeSerializableElement(descriptor, 0, serializer<RpcQueryResponse.AccountView>(), value)
      is RpcQueryResponse.ContractCodeView -> out.encodeSerializableElement(descriptor, 1, serializer<RpcQueryResponse.ContractCodeView>(), value)
      is RpcQueryResponse.ViewStateResult -> out.encodeSerializableElement(descriptor, 2, serializer<RpcQueryResponse.ViewStateResult>(), value)
      is RpcQueryResponse.CallResult -> out.encodeSerializableElement(descriptor, 3, serializer<RpcQueryResponse.CallResult>(), value)
      is RpcQueryResponse.AccessKeyView -> out.encodeSerializableElement(descriptor, 4, serializer<RpcQueryResponse.AccessKeyView>(), value)
      is RpcQueryResponse.AccessKeyList -> out.encodeSerializableElement(descriptor, 5, serializer<RpcQueryResponse.AccessKeyList>(), value)
    }
          ...
          if (jobj.size == 1) {
            ...
            when (key) {
              "AccountView" -> {
                ...
                return decoder.json.decodeFromJsonElement(serializer<RpcQueryResponse.AccountView>(), obj)
              }
              "ContractCodeView" -> {
                val obj = valueElem as? JsonObject ?: throw SerializationException("Expected object payload for variant " + key)
                return decoder.json.decodeFromJsonElement(serializer<RpcQueryResponse.ContractCodeView>(), obj)
              }
              "ViewStateResult" -> {
                val obj = valueElem as? JsonObject ?: throw SerializationException("Expected object payload for variant " + key)
                return decoder.json.decodeFromJsonElement(serializer<RpcQueryResponse.ViewStateResult>(), obj)
              }
              "CallResult" -> {
                val obj = valueElem as? JsonObject ?: throw SerializationException("Expected object payload for variant " + key)
                return decoder.json.decodeFromJsonElement(serializer<RpcQueryResponse.CallResult>(), obj)
              }
              "AccessKeyView" -> {
                val obj = valueElem as? JsonObject ?: throw SerializationException("Expected object payload for variant " + key)
                return decoder.json.decodeFromJsonElement(serializer<RpcQueryResponse.AccessKeyView>(), obj)
              }
              "AccessKeyList" -> {
                val obj = valueElem as? JsonObject ?: throw SerializationException("Expected object payload for variant " + key)
                return decoder.json.decodeFromJsonElement(serializer<RpcQueryResponse.AccessKeyList>(), obj)
              }
            ...
            if (typeField == null) throw SerializationException("Missing discriminator or recognizable variant in RpcQueryResponse")
            ...
            when (tf) {
              ...
              else -> throw SerializationException("Unknown type discriminator for RpcQueryResponse: " + tf)
            }
          }
        }
      }
