...
public object ReceiptEnumViewSerializer : KSerializer<ReceiptEnumView> {
  ...
  override fun serialize(encoder: Encoder, `value`: ReceiptEnumView) {
    ...
    when (value) {
      is ReceiptEnumView.Action -> out.encodeSerializableElement(descriptor, 0, serializer<ReceiptEnumView.Action.ActionPayload>(), value.action)
      is ReceiptEnumView.Data -> out.encodeSerializableElement(descriptor, 1, serializer<ReceiptEnumView.Data.DataPayload>(), value.data)
      is ReceiptEnumView.GlobalContractDistribution -> out.encodeSerializableElement(descriptor, 2, serializer<ReceiptEnumView.GlobalContractDistribution.GlobalContractDistributionPayload>(), value.globalContractDistribution)
    }
          ...
          if (jobj.size == 1) {
            ...
            when (key) {
              "Action" -> {
                ...
                val actionVal = decoder.json.decodeFromJsonElement(serializer<ReceiptEnumView.Action.ActionPayload>(), obj["Action"] ?: throw SerializationException("Missing field 'Action' for variant " + key))
                return ReceiptEnumView.Action(actionVal)
              }
              "Data" -> {
                val obj = valueElem as? JsonObject ?: throw SerializationException("Expected object payload for variant " + key)
                val dataVal = decoder.json.decodeFromJsonElement(serializer<ReceiptEnumView.Data.DataPayload>(), obj["Data"] ?: throw SerializationException("Missing field 'Data' for variant " + key))
                return ReceiptEnumView.Data(dataVal)
              }
              "GlobalContractDistribution" -> {
                val obj = valueElem as? JsonObject ?: throw SerializationException("Expected object payload for variant " + key)
                val globalContractDistributionVal = decoder.json.decodeFromJsonElement(serializer<ReceiptEnumView.GlobalContractDistribution.GlobalContractDistributionPayload>(), obj["GlobalContractDistribution"] ?: throw SerializationException("Missing field 'GlobalContractDistribution' for variant " + key))
                return ReceiptEnumView.GlobalContractDistribution(globalContractDistributionVal)
              }
            ...
            val typeField = jobj["type"]?.jsonPrimitive?.contentOrNull ?: throw SerializationException("Missing 'type' discriminator in ReceiptEnumView")
            when (typeField) {
              "Action" -> {
              ...
              else -> throw SerializationException("Unknown type discriminator for ReceiptEnumView: " + typeField)
            }
          }
        }
      }
