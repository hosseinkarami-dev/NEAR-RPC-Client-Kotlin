      ...
      when (element) {
        ...
        is JsonObject -> {
          ...
          if (jobj["ActionError"] != null) {
            return TxExecutionError.ActionError(decoder.json.decodeFromJsonElement(serializer<ActionError>(), jobj["ActionError"]!!))
          }
          if (jobj["InvalidTxError"] != null) {
            return TxExecutionError.InvalidTxError(decoder.json.decodeFromJsonElement(serializer<InvalidTxError>(), jobj["InvalidTxError"]!!))
          }
          if (jobj.size == 1) {
            ...
            when (key) {
              "ActionError" -> {
                ...
                return decoder.json.decodeFromJsonElement(serializer<TxExecutionError.ActionError>(), obj)
              }
              "InvalidTxError" -> {
                val obj = valueElem as? JsonObject ?: throw SerializationException("Expected object payload for variant " + key)
                return decoder.json.decodeFromJsonElement(serializer<TxExecutionError.InvalidTxError>(), obj)
              }
