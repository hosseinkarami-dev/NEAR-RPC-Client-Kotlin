...
public object JsonRpcResponseForArrayOfValidatorStakeViewAndRpcErrorSerializer : KSerializer<JsonRpcResponseForArrayOfValidatorStakeViewAndRpcError> {
  ...
  override fun serialize(encoder: Encoder, `value`: JsonRpcResponseForArrayOfValidatorStakeViewAndRpcError) {
    ...
    when (value) {
      is JsonRpcResponseForArrayOfValidatorStakeViewAndRpcError.Result -> out.encodeSerializableElement(descriptor, 0, serializer<JsonRpcResponseForArrayOfValidatorStakeViewAndRpcError.Result>(), value)
      is JsonRpcResponseForArrayOfValidatorStakeViewAndRpcError.Error -> out.encodeSerializableElement(descriptor, 1, serializer<JsonRpcResponseForArrayOfValidatorStakeViewAndRpcError.Error>(), value)
    }
          ...
          if (jobj.size == 1) {
            ...
            when (key) {
              "result" -> {
                ...
                return decoder.json.decodeFromJsonElement(serializer<JsonRpcResponseForArrayOfValidatorStakeViewAndRpcError.Result>(), obj)
              }
              "error" -> {
                val obj = valueElem as? JsonObject ?: throw SerializationException("Expected object payload for variant " + key)
                return decoder.json.decodeFromJsonElement(serializer<JsonRpcResponseForArrayOfValidatorStakeViewAndRpcError.Error>(), obj)
              }
            ...
            if (typeField == null) throw SerializationException("Missing discriminator (one of id/jsonrpc) or recognizable variant in JsonRpcResponseForArrayOfValidatorStakeViewAndRpcError")
