          ...
          if (jobj.size == 1) {
            ...
            when (key) {
              "account_touched" -> {
                ...
                return decoder.json.decodeFromJsonElement(serializer<StateChangeKindView.AccountTouched>(), obj)
              }
              "access_key_touched" -> {
                val obj = valueElem as? JsonObject ?: throw SerializationException("Expected object payload for variant " + key)
                return decoder.json.decodeFromJsonElement(serializer<StateChangeKindView.AccessKeyTouched>(), obj)
              }
              "data_touched" -> {
                val obj = valueElem as? JsonObject ?: throw SerializationException("Expected object payload for variant " + key)
                return decoder.json.decodeFromJsonElement(serializer<StateChangeKindView.DataTouched>(), obj)
              }
              "contract_code_touched" -> {
                val obj = valueElem as? JsonObject ?: throw SerializationException("Expected object payload for variant " + key)
                return decoder.json.decodeFromJsonElement(serializer<StateChangeKindView.ContractCodeTouched>(), obj)
              }
            ...
            if (typeField == null) throw SerializationException("Missing discriminator (one of type) or recognizable variant in StateChangeKindView")
