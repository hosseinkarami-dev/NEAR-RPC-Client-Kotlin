...
public object JsonRpcResponseForRpcLightClientExecutionProofResponseAndRpcErrorSerializer : KSerializer<JsonRpcResponseForRpcLightClientExecutionProofResponseAndRpcError> {
  ...
  override fun serialize(encoder: Encoder, `value`: JsonRpcResponseForRpcLightClientExecutionProofResponseAndRpcError) {
    ...
    when (value) {
      is JsonRpcResponseForRpcLightClientExecutionProofResponseAndRpcError.Result -> out.encodeSerializableElement(descriptor, 0, serializer<JsonRpcResponseForRpcLightClientExecutionProofResponseAndRpcError.Result>(), value)
      is JsonRpcResponseForRpcLightClientExecutionProofResponseAndRpcError.Error -> out.encodeSerializableElement(descriptor, 1, serializer<JsonRpcResponseForRpcLightClientExecutionProofResponseAndRpcError.Error>(), value)
    }
          ...
          if (jobj.size == 1) {
            ...
            when (key) {
              "result" -> {
                ...
                return decoder.json.decodeFromJsonElement(serializer<JsonRpcResponseForRpcLightClientExecutionProofResponseAndRpcError.Result>(), obj)
              }
              "error" -> {
                val obj = valueElem as? JsonObject ?: throw SerializationException("Expected object payload for variant " + key)
                return decoder.json.decodeFromJsonElement(serializer<JsonRpcResponseForRpcLightClientExecutionProofResponseAndRpcError.Error>(), obj)
              }
            ...
            if (typeField == null) throw SerializationException("Missing discriminator (one of id/jsonrpc) or recognizable variant in JsonRpcResponseForRpcLightClientExecutionProofResponseAndRpcError")
