...
public object RpcChunkRequestSerializer : KSerializer<RpcChunkRequest> {
  ...
  override fun serialize(encoder: Encoder, `value`: RpcChunkRequest) {
    ...
    when (value) {
      is RpcChunkRequest.BlockShardId -> out.encodeSerializableElement(descriptor, 0, serializer<BlockId>(), value.blockId)
      is RpcChunkRequest.ChunkHash -> out.encodeSerializableElement(descriptor, 1, serializer<CryptoHash>(), value.chunkId)
    }
      ...
      when (element) {
        ...
        is JsonObject -> {
          ...
          if (jobj.size == 1) {
            ...
            val typeField = jobj["type"]?.jsonPrimitive?.contentOrNull ?: throw SerializationException("Missing 'type' discriminator in RpcChunkRequest")
            when (typeField) {
              "block_shard_id" -> {
              ...
              else -> throw SerializationException("Unknown type discriminator for RpcChunkRequest: " + typeField)
            }
          }
        }
      }
