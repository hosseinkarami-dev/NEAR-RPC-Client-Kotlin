      ...
      when (element) {
        ...
        is JsonObject -> {
          ...
          if (jobj.size == 1) {
            ...
            val typeField = jobj["type"]?.jsonPrimitive?.contentOrNull ?: throw SerializationException("Missing 'type' discriminator in ExternalStorageLocation")
            when (typeField) {
              "S3" -> {
              ...
              else -> throw SerializationException("Unknown type discriminator for ExternalStorageLocation: " + typeField)
            }
          }
        }
      }
