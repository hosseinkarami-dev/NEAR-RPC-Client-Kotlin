...
public object GlobalContractIdentifierViewSerializer : KSerializer<GlobalContractIdentifierView> {
  ...
  override fun serialize(encoder: Encoder, `value`: GlobalContractIdentifierView) {
    ...
    when (value) {
      is GlobalContractIdentifierView.CryptoHash -> out.encodeSerializableElement(descriptor, 0, serializer<CryptoHash>(), value.value)
      is GlobalContractIdentifierView.AccountId -> out.encodeSerializableElement(descriptor, 1, serializer<AccountId>(), value.value)
    }
      ...
      when (element) {
        ...
        is JsonObject -> {
          ...
          if (jobj.size == 1) {
            ...
            val typeField = jobj["type"]?.jsonPrimitive?.contentOrNull ?: throw SerializationException("Missing 'type' discriminator in GlobalContractIdentifierView")
            when (typeField) {
              "CryptoHash" -> {
              ...
              else -> throw SerializationException("Unknown type discriminator for GlobalContractIdentifierView: " + typeField)
            }
          }
        }
      }
