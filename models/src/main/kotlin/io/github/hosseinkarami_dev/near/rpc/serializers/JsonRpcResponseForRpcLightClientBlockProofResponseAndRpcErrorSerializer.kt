...
public object JsonRpcResponseForRpcLightClientBlockProofResponseAndRpcErrorSerializer : KSerializer<JsonRpcResponseForRpcLightClientBlockProofResponseAndRpcError> {
  ...
  override fun serialize(encoder: Encoder, `value`: JsonRpcResponseForRpcLightClientBlockProofResponseAndRpcError) {
    ...
    when (value) {
      is JsonRpcResponseForRpcLightClientBlockProofResponseAndRpcError.Result -> out.encodeSerializableElement(descriptor, 0, serializer<JsonRpcResponseForRpcLightClientBlockProofResponseAndRpcError.Result>(), value)
      is JsonRpcResponseForRpcLightClientBlockProofResponseAndRpcError.Error -> out.encodeSerializableElement(descriptor, 1, serializer<JsonRpcResponseForRpcLightClientBlockProofResponseAndRpcError.Error>(), value)
    }
          ...
          if (jobj.size == 1) {
            ...
            when (key) {
              "result" -> {
                ...
                return decoder.json.decodeFromJsonElement(serializer<JsonRpcResponseForRpcLightClientBlockProofResponseAndRpcError.Result>(), obj)
              }
              "error" -> {
                val obj = valueElem as? JsonObject ?: throw SerializationException("Expected object payload for variant " + key)
                return decoder.json.decodeFromJsonElement(serializer<JsonRpcResponseForRpcLightClientBlockProofResponseAndRpcError.Error>(), obj)
              }
            ...
            if (typeField == null) throw SerializationException("Missing discriminator (one of id/jsonrpc) or recognizable variant in JsonRpcResponseForRpcLightClientBlockProofResponseAndRpcError")
            ...
            when (tf) {
              ...
              else -> throw SerializationException("Unknown type discriminator for JsonRpcResponseForRpcLightClientBlockProofResponseAndRpcError: " + tf)
            }
          }
        }
      }
