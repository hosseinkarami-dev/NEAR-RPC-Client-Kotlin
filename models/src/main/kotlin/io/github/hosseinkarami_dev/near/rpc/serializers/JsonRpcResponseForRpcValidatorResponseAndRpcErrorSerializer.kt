      ...
      when (element) {
        ...
        is JsonObject -> {
          ...
          if (jobj.size == 1) {
            ...
            val typeField = jobj["type"]?.jsonPrimitive?.contentOrNull ?: throw SerializationException("Missing 'type' discriminator in JsonRpcResponseForRpcValidatorResponseAndRpcError")
            when (typeField) {
              "result" -> {
              ...
              else -> throw SerializationException("Unknown type discriminator for JsonRpcResponseForRpcValidatorResponseAndRpcError: " + typeField)
            }
          }
        }
      }
