...
public object JsonRpcResponseForRpcValidatorResponseAndRpcErrorSerializer : KSerializer<JsonRpcResponseForRpcValidatorResponseAndRpcError> {
  ...
  override fun serialize(encoder: Encoder, `value`: JsonRpcResponseForRpcValidatorResponseAndRpcError) {
    ...
    when (value) {
      is JsonRpcResponseForRpcValidatorResponseAndRpcError.Result -> out.encodeSerializableElement(descriptor, 0, serializer<JsonRpcResponseForRpcValidatorResponseAndRpcError.Result>(), value)
      is JsonRpcResponseForRpcValidatorResponseAndRpcError.Error -> out.encodeSerializableElement(descriptor, 1, serializer<JsonRpcResponseForRpcValidatorResponseAndRpcError.Error>(), value)
    }
          ...
          if (jobj.size == 1) {
            ...
            when (key) {
              "result" -> {
                ...
                return decoder.json.decodeFromJsonElement(serializer<JsonRpcResponseForRpcValidatorResponseAndRpcError.Result>(), obj)
              }
              "error" -> {
                val obj = valueElem as? JsonObject ?: throw SerializationException("Expected object payload for variant " + key)
                return decoder.json.decodeFromJsonElement(serializer<JsonRpcResponseForRpcValidatorResponseAndRpcError.Error>(), obj)
              }
            ...
            if (typeField == null) throw SerializationException("Missing discriminator (one of id/jsonrpc) or recognizable variant in JsonRpcResponseForRpcValidatorResponseAndRpcError")
