...
public object GlobalContractDeployModeSerializer : KSerializer<GlobalContractDeployMode> {
  ...
  override fun serialize(encoder: Encoder, `value`: GlobalContractDeployMode) {
    ...
    when (value) {
      GlobalContractDeployMode.CodeHash -> out.encodeStringElement(descriptor, 0, "CodeHash")
      GlobalContractDeployMode.AccountId -> out.encodeStringElement(descriptor, 1, "AccountId")
    }
      ...
      when (element) {
        is JsonPrimitive -> {
          if (element.isString) {
            ...
            throw SerializationException("Unknown discriminator string for GlobalContractDeployMode: " + s)
          }
        }
        is JsonArray -> {
        ...
        is JsonObject -> {
          ...
          if (jobj.size == 1) {
            ...
            val typeField = jobj["type"]?.jsonPrimitive?.contentOrNull ?: throw SerializationException("Missing 'type' discriminator in GlobalContractDeployMode")
            when (typeField) {
              "CodeHash" -> return GlobalContractDeployMode.CodeHash
              "AccountId" -> return GlobalContractDeployMode.AccountId
              else -> throw SerializationException("Unknown type discriminator for GlobalContractDeployMode: " + typeField)
            }
          }
        }
      }
