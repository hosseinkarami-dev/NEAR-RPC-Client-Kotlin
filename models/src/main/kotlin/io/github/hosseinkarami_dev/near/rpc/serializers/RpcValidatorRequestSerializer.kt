...
public object RpcValidatorRequestSerializer : KSerializer<RpcValidatorRequest> {
  ...
  override fun serialize(encoder: Encoder, `value`: RpcValidatorRequest) {
    ...
    when (value) {
      RpcValidatorRequest.Latest -> out.encodeStringElement(descriptor, 0, "latest")
      is RpcValidatorRequest.EpochId -> out.encodeSerializableElement(descriptor, 1, serializer<EpochId>(), value.epochId)
      is RpcValidatorRequest.BlockId -> out.encodeSerializableElement(descriptor, 2, serializer<BlockId>(), value.blockId)
    }
          ...
          if (jobj.size == 1) {
            ...
            when (key) {
              "epoch_id" -> {
                ...
                val epochIdVal = decoder.json.decodeFromJsonElement(serializer<EpochId>(), obj["epoch_id"] ?: throw SerializationException("Missing field 'epoch_id' for variant " + key))
                return RpcValidatorRequest.EpochId(epochIdVal)
              }
              "block_id" -> {
                val obj = valueElem as? JsonObject ?: throw SerializationException("Expected object payload for variant " + key)
            ...
            val typeField = jobj["type"]?.jsonPrimitive?.contentOrNull ?: throw SerializationException("Missing 'type' discriminator in RpcValidatorRequest")
            when (typeField) {
              "epoch_id" -> {
              ...
              else -> throw SerializationException("Unknown type discriminator for RpcValidatorRequest: " + typeField)
            }
          }
        }
      }
