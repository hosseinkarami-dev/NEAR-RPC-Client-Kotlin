...
public object ReceiptValidationErrorSerializer : KSerializer<ReceiptValidationError> {
  ...
  override fun serialize(encoder: Encoder, `value`: ReceiptValidationError) {
    ...
    when (value) {
      is ReceiptValidationError.InvalidPredecessorId -> out.encodeSerializableElement(descriptor, 0, serializer<ReceiptValidationError.InvalidPredecessorId.InvalidPredecessorIdPayload>(), value.invalidPredecessorId)
      is ReceiptValidationError.InvalidReceiverId -> out.encodeSerializableElement(descriptor, 1, serializer<ReceiptValidationError.InvalidReceiverId.InvalidReceiverIdPayload>(), value.invalidReceiverId)
      is ReceiptValidationError.InvalidSignerId -> out.encodeSerializableElement(descriptor, 2, serializer<ReceiptValidationError.InvalidSignerId.InvalidSignerIdPayload>(), value.invalidSignerId)
      is ReceiptValidationError.InvalidDataReceiverId -> out.encodeSerializableElement(descriptor, 3, serializer<ReceiptValidationError.InvalidDataReceiverId.InvalidDataReceiverIdPayload>(), value.invalidDataReceiverId)
      is ReceiptValidationError.ReturnedValueLengthExceeded -> out.encodeSerializableElement(descriptor, 4, serializer<ReceiptValidationError.ReturnedValueLengthExceeded.ReturnedValueLengthExceededPayload>(), value.returnedValueLengthExceeded)
      is ReceiptValidationError.NumberInputDataDependenciesExceeded -> out.encodeSerializableElement(descriptor, 5, serializer<ReceiptValidationError.NumberInputDataDependenciesExceeded.NumberInputDataDependenciesExceededPayload>(), value.numberInputDataDependenciesExceeded)
      is ReceiptValidationError.ActionsValidation -> out.encodeSerializableElement(descriptor, 6, serializer<ActionsValidationError>(), value.actionsValidation)
      is ReceiptValidationError.ReceiptSizeExceeded -> out.encodeSerializableElement(descriptor, 7, serializer<ReceiptValidationError.ReceiptSizeExceeded.ReceiptSizeExceededPayload>(), value.receiptSizeExceeded)
    }
      ...
      when (element) {
        ...
        is JsonObject -> {
          ...
          if (jobj.size == 1) {
            ...
            val typeField = jobj["type"]?.jsonPrimitive?.contentOrNull ?: throw SerializationException("Missing 'type' discriminator in ReceiptValidationError")
            when (typeField) {
              "InvalidPredecessorId" -> {
              ...
              else -> throw SerializationException("Unknown type discriminator for ReceiptValidationError: " + typeField)
            }
          }
        }
      }
