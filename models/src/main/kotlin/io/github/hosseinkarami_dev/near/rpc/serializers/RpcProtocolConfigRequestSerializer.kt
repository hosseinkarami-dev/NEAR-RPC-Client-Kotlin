...
public object RpcProtocolConfigRequestSerializer : KSerializer<RpcProtocolConfigRequest> {
  ...
  override fun serialize(encoder: Encoder, `value`: RpcProtocolConfigRequest) {
    ...
    when (value) {
      is RpcProtocolConfigRequest.BlockId -> out.encodeSerializableElement(descriptor, 0, serializer<RpcProtocolConfigRequest.BlockId>(), value)
      is RpcProtocolConfigRequest.Finality -> out.encodeSerializableElement(descriptor, 1, serializer<RpcProtocolConfigRequest.Finality>(), value)
      is RpcProtocolConfigRequest.SyncCheckpoint -> out.encodeSerializableElement(descriptor, 2, serializer<RpcProtocolConfigRequest.SyncCheckpoint>(), value)
    }
      ...
      when (element) {
        ...
        is JsonObject -> {
          ...
          if (jobj["block_id"] != null) {
            return RpcProtocolConfigRequest.BlockId(decoder.json.decodeFromJsonElement(serializer<BlockId>(), jobj["block_id"]!!))
          }
          if (jobj["finality"] != null) {
            return RpcProtocolConfigRequest.Finality(decoder.json.decodeFromJsonElement(serializer<Finality>(), jobj["finality"]!!))
          }
          if (jobj["sync_checkpoint"] != null) {
            return RpcProtocolConfigRequest.SyncCheckpoint(decoder.json.decodeFromJsonElement(serializer<SyncCheckpoint>(), jobj["sync_checkpoint"]!!))
          }
          if (jobj.size == 1) {
            ...
            when (key) {
              "block_id" -> {
                ...
                return decoder.json.decodeFromJsonElement(serializer<RpcProtocolConfigRequest.BlockId>(), obj)
              }
              "finality" -> {
                val obj = valueElem as? JsonObject ?: throw SerializationException("Expected object payload for variant " + key)
                return decoder.json.decodeFromJsonElement(serializer<RpcProtocolConfigRequest.Finality>(), obj)
              }
              "sync_checkpoint" -> {
                val obj = valueElem as? JsonObject ?: throw SerializationException("Expected object payload for variant " + key)
                return decoder.json.decodeFromJsonElement(serializer<RpcProtocolConfigRequest.SyncCheckpoint>(), obj)
              }
            ...
            if (typeField == null) throw SerializationException("Missing discriminator or recognizable variant in RpcProtocolConfigRequest")
