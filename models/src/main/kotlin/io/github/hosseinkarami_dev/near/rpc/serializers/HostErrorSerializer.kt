      ...
      when (element) {
        ...
        is JsonObject -> {
          ...
          if (jobj.size == 1) {
            ...
            val typeField = jobj["type"]?.jsonPrimitive?.contentOrNull ?: throw SerializationException("Missing 'type' discriminator in HostError")
            when (typeField) {
              "GuestPanic" -> {
              ...
              else -> throw SerializationException("Unknown type discriminator for HostError: " + typeField)
            }
          }
        }
      }
