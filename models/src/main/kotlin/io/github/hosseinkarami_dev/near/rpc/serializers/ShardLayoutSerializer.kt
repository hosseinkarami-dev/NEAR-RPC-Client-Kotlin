      ...
      when (element) {
        ...
        is JsonObject -> {
          ...
          if (jobj.size == 1) {
            ...
            val typeField = jobj["type"]?.jsonPrimitive?.contentOrNull ?: throw SerializationException("Missing 'type' discriminator in ShardLayout")
            when (typeField) {
              "V0" -> {
              ...
              else -> throw SerializationException("Unknown type discriminator for ShardLayout: " + typeField)
            }
          }
        }
      }
