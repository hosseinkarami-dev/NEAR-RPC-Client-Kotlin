      ...
      when (element) {
        ...
        is JsonObject -> {
          ...
          if (jobj.size == 1) {
            ...
            val typeField = jobj["type"]?.jsonPrimitive?.contentOrNull ?: throw SerializationException("Missing 'type' discriminator in CompilationError")
            when (typeField) {
              "CodeDoesNotExist" -> {
              ...
              else -> throw SerializationException("Unknown type discriminator for CompilationError: " + typeField)
            }
          }
        }
      }
