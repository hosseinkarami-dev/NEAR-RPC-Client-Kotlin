  ...
  override fun deserialize(decoder: Decoder): RpcTransactionResponse {
    if (decoder is JsonDecoder) {
      ...
      when (element) {
        ...
        is JsonObject -> {
          val jobj = element
          if (listOf("final_execution_status", "receipts", "receipts_outcome", "status", "transaction", "transaction_outcome").all { jobj[it] != null }) {
            return decoder.json.decodeFromJsonElement(serializer<RpcTransactionResponse.FinalExecutionOutcomeWithReceiptView>(), jobj)
          }
          if (listOf("final_execution_status", "receipts_outcome", "status", "transaction", "transaction_outcome").all { jobj[it] != null }) {
