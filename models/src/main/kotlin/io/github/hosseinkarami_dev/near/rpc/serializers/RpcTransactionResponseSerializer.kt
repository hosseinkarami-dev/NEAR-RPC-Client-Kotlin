...
public object RpcTransactionResponseSerializer : KSerializer<RpcTransactionResponse> {
  ...
  override fun serialize(encoder: Encoder, `value`: RpcTransactionResponse) {
    ...
    when (value) {
      is RpcTransactionResponse.FinalExecutionOutcomeWithReceiptView -> out.encodeSerializableElement(descriptor, 0, ListSerializer(serializer<ReceiptView>()), value.receipts)
      is RpcTransactionResponse.FinalExecutionOutcomeView -> out.encodeSerializableElement(descriptor, 1, ListSerializer(serializer<ExecutionOutcomeWithIdView>()), value.receiptsOutcome)
    }
      ...
      when (element) {
        is JsonPrimitive -> {
          if (element.isString) {
            ...
            throw SerializationException("Unknown discriminator string for RpcTransactionResponse: " + s)
          }
        }
        is JsonArray -> {
          ...
          if (jobj.size == 1) {
            ...
            when (key) {
              "FinalExecutionOutcomeWithReceiptView" -> {
                ...
                val receiptsVal = decoder.json.decodeFromJsonElement(ListSerializer(serializer<ReceiptView>()), obj["receipts"] ?: throw SerializationException("Missing field 'receipts' for variant " + key))
                val receiptsOutcomeVal = decoder.json.decodeFromJsonElement(ListSerializer(serializer<ExecutionOutcomeWithIdView>()), obj["receipts_outcome"] ?: throw SerializationException("Missing field 'receipts_outcome' for variant " + key))
                val statusVal = decoder.json.decodeFromJsonElement(serializer<FinalExecutionStatus>(), obj["status"] ?: throw SerializationException("Missing field 'status' for variant " + key))
                val transactionVal = decoder.json.decodeFromJsonElement(serializer<SignedTransactionView>(), obj["transaction"] ?: throw SerializationException("Missing field 'transaction' for variant " + key))
                val transactionOutcomeVal = decoder.json.decodeFromJsonElement(serializer<ExecutionOutcomeWithIdView>(), obj["transaction_outcome"] ?: throw SerializationException("Missing field 'transaction_outcome' for variant " + key))
                val finalExecutionStatusVal = decoder.json.decodeFromJsonElement(serializer<TxExecutionStatus>(), obj["final_execution_status"] ?: throw SerializationException("Missing field 'final_execution_status' for variant " + key))
              ...
              "FinalExecutionOutcomeView" -> {
                ...
                val transactionOutcomeVal = decoder.json.decodeFromJsonElement(serializer<ExecutionOutcomeWithIdView>(), obj["transaction_outcome"] ?: throw SerializationException("Missing field 'transaction_outcome' for variant " + key))
                val finalExecutionStatusVal = decoder.json.decodeFromJsonElement(serializer<TxExecutionStatus>(), obj["final_execution_status"] ?: throw SerializationException("Missing field 'final_execution_status' for variant " + key))
                return RpcTransactionResponse.FinalExecutionOutcomeView(receiptsOutcomeVal, statusVal, transactionVal, transactionOutcomeVal, finalExecutionStatusVal)
              }
              else -> throw SerializationException("Unknown discriminator key for RpcTransactionResponse: " + key)
            }
          }
          else {
            val typeField = jobj["type"]?.jsonPrimitive?.contentOrNull ?: throw SerializationException("Missing 'type' discriminator in RpcTransactionResponse")
            when (typeField) {
              "FinalExecutionOutcomeWithReceiptView" -> {
              ...
              else -> throw SerializationException("Unknown type discriminator for RpcTransactionResponse: " + typeField)
            }
          }
        }
      }
