      ...
      when (element) {
        ...
        is JsonObject -> {
          ...
          if (jobj.size == 1) {
            ...
            val typeField = jobj["type"]?.jsonPrimitive?.contentOrNull ?: throw SerializationException("Missing 'type' discriminator in RpcCongestionLevelRequest")
            when (typeField) {
              "block_shard_id" -> {
              ...
              else -> throw SerializationException("Unknown type discriminator for RpcCongestionLevelRequest: " + typeField)
            }
          }
        }
      }
