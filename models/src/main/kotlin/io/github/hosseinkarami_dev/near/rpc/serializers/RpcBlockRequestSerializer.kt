...
public object RpcBlockRequestSerializer : KSerializer<RpcBlockRequest> {
  ...
  override fun serialize(encoder: Encoder, `value`: RpcBlockRequest) {
    ...
    when (value) {
      is RpcBlockRequest.BlockId -> out.encodeSerializableElement(descriptor, 0, serializer<BlockId>(), value.blockId)
      is RpcBlockRequest.Finality -> out.encodeSerializableElement(descriptor, 1, serializer<Finality>(), value.finality)
      is RpcBlockRequest.SyncCheckpoint -> out.encodeSerializableElement(descriptor, 2, serializer<SyncCheckpoint>(), value.syncCheckpoint)
    }
      ...
      when (element) {
        is JsonPrimitive -> {
          if (element.isString) {
            ...
            throw SerializationException("Unknown discriminator string for RpcBlockRequest: " + s)
          }
        }
        is JsonArray -> {
        ...
        is JsonObject -> {
          ...
          if (jobj["block_id"] != null) {
            ...
            return RpcBlockRequest.BlockId(blockIdVal)
          }
          if (jobj["finality"] != null) {
            ...
            return RpcBlockRequest.Finality(finalityVal)
          }
          if (jobj["sync_checkpoint"] != null) {
            ...
            return RpcBlockRequest.SyncCheckpoint(syncCheckpointVal)
          }
          if (jobj.size == 1) {
            ...
            val typeField = jobj["type"]?.jsonPrimitive?.contentOrNull ?: throw SerializationException("Missing 'type' discriminator in RpcBlockRequest")
            when (typeField) {
              "block_id" -> {
              ...
              else -> throw SerializationException("Unknown type discriminator for RpcBlockRequest: " + typeField)
            }
          }
        }
      }
