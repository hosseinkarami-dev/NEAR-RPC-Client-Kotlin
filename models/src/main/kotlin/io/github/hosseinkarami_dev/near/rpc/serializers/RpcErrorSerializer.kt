  ...
  override fun deserialize(decoder: Decoder): RpcError {
    if (decoder is JsonDecoder) {
      ...
      when (element) {
        ...
        is JsonObject -> {
          val jobj = element
          if (listOf("cause", "code", "message", "name").all { jobj[it] != null }) {
          ...
          if (jobj.size == 1) {
            ...
            val discriminatorCandidates = listOf("message")
            ...
            if (typeField == null) throw SerializationException("Missing discriminator (one of message) or recognizable variant in RpcError")
