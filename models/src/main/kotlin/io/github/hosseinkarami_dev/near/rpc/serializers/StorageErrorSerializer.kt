...
public object StorageErrorSerializer : KSerializer<StorageError> {
  ...
  override fun serialize(encoder: Encoder, `value`: StorageError) {
    ...
    when (value) {
      StorageError.StorageInternalError -> out.encodeStringElement(descriptor, 0, "StorageInternalError")
      is StorageError.MissingTrieValue -> out.encodeSerializableElement(descriptor, 1, serializer<MissingTrieValue>(), value.missingTrieValue)
      StorageError.UnexpectedTrieValue -> out.encodeStringElement(descriptor, 2, "UnexpectedTrieValue")
      is StorageError.StorageInconsistentState -> out.encodeSerializableElement(descriptor, 3, serializer<String>(), value.storageInconsistentState)
      is StorageError.FlatStorageBlockNotSupported -> out.encodeSerializableElement(descriptor, 4, serializer<String>(), value.flatStorageBlockNotSupported)
      is StorageError.MemTrieLoadingError -> out.encodeSerializableElement(descriptor, 5, serializer<String>(), value.memTrieLoadingError)
    }
      ...
      when (element) {
        ...
        is JsonObject -> {
          ...
          if (jobj.size == 1) {
            ...
            val typeField = jobj["type"]?.jsonPrimitive?.contentOrNull ?: throw SerializationException("Missing 'type' discriminator in StorageError")
            when (typeField) {
              "MissingTrieValue" -> {
              ...
              else -> throw SerializationException("Unknown type discriminator for StorageError: " + typeField)
            }
          }
        }
      }
