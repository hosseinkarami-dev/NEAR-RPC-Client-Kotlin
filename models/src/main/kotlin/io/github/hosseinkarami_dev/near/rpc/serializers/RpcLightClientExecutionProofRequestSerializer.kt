...
public object RpcLightClientExecutionProofRequestSerializer : KSerializer<RpcLightClientExecutionProofRequest> {
  ...
  override fun serialize(encoder: Encoder, `value`: RpcLightClientExecutionProofRequest) {
    ...
    when (value) {
      is RpcLightClientExecutionProofRequest.Transaction -> out.encodeSerializableElement(descriptor, 0, serializer<AccountId>(), value.senderId)
      is RpcLightClientExecutionProofRequest.Receipt -> out.encodeSerializableElement(descriptor, 1, serializer<CryptoHash>(), value.receiptId)
    }
      ...
      when (element) {
        ...
        is JsonObject -> {
          ...
          if (jobj.size == 1) {
            ...
            val typeField = jobj["type"]?.jsonPrimitive?.contentOrNull ?: throw SerializationException("Missing 'type' discriminator in RpcLightClientExecutionProofRequest")
            when (typeField) {
              "transaction" -> {
              ...
              else -> throw SerializationException("Unknown type discriminator for RpcLightClientExecutionProofRequest: " + typeField)
            }
          }
        }
      }
