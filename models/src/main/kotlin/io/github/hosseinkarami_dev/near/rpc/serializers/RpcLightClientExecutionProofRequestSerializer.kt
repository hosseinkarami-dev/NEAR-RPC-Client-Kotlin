          ...
          if (jobj.size == 1) {
            ...
            when (key) {
              "transaction" -> {
                ...
                return decoder.json.decodeFromJsonElement(serializer<RpcLightClientExecutionProofRequest.Transaction>(), obj)
              }
              "receipt" -> {
                val obj = valueElem as? JsonObject ?: throw SerializationException("Expected object payload for variant " + key)
                return decoder.json.decodeFromJsonElement(serializer<RpcLightClientExecutionProofRequest.Receipt>(), obj)
              }
            ...
            if (typeField == null) throw SerializationException("Missing discriminator (one of type) or recognizable variant in RpcLightClientExecutionProofRequest")
