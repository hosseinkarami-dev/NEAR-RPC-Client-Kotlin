...
public object RpcTransactionStatusRequestSerializer : KSerializer<RpcTransactionStatusRequest> {
  ...
  override fun serialize(encoder: Encoder, `value`: RpcTransactionStatusRequest) {
    ...
    when (value) {
      is RpcTransactionStatusRequest.SignedTxBase64 -> out.encodeSerializableElement(descriptor, 0, serializer<SignedTransaction>(), value.signedTxBase64)
      is RpcTransactionStatusRequest.SenderAccountId -> out.encodeSerializableElement(descriptor, 1, serializer<AccountId>(), value.senderAccountId)
    }
      ...
      when (element) {
        ...
        is JsonObject -> {
          ...
          if (jobj.size == 1) {
            ...
            val typeField = jobj["type"]?.jsonPrimitive?.contentOrNull ?: throw SerializationException("Missing 'type' discriminator in RpcTransactionStatusRequest")
            when (typeField) {
              "signed_tx_base64" -> {
              ...
              else -> throw SerializationException("Unknown type discriminator for RpcTransactionStatusRequest: " + typeField)
            }
          }
        }
      }
