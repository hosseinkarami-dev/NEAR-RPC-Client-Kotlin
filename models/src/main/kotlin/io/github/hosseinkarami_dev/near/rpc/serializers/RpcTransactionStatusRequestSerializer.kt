          ...
          if (jobj.size == 1) {
            ...
            when (key) {
              "signed_tx_base64" -> {
                ...
                return decoder.json.decodeFromJsonElement(serializer<RpcTransactionStatusRequest.SignedTxBase64>(), obj)
              }
              "sender_account_id" -> {
                val obj = valueElem as? JsonObject ?: throw SerializationException("Expected object payload for variant " + key)
                return decoder.json.decodeFromJsonElement(serializer<RpcTransactionStatusRequest.SenderAccountId>(), obj)
              }
