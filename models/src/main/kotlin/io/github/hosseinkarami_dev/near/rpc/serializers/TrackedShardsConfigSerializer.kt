...
public object TrackedShardsConfigSerializer : KSerializer<TrackedShardsConfig> {
  ...
  override fun serialize(encoder: Encoder, `value`: TrackedShardsConfig) {
    if (encoder is JsonEncoder) {
      ...
      when (value) {
        is TrackedShardsConfig.NoShards -> {
        ...
        is TrackedShardsConfig.AllShards -> {
    ...
    if (decoder is JsonDecoder) {
      ...
      when (element) {
        ...
        is JsonObject -> {
          val jobj = element
          if (jobj["Shards"] != null) {
            return TrackedShardsConfig.Shards(decoder.json.decodeFromJsonElement(ListSerializer(serializer<ShardUId>()), jobj["Shards"]!!))
          }
          if (jobj["ShadowValidator"] != null) {
            return TrackedShardsConfig.ShadowValidator(decoder.json.decodeFromJsonElement(serializer<AccountId>(), jobj["ShadowValidator"]!!))
          }
          if (jobj["Schedule"] != null) {
            return TrackedShardsConfig.Schedule(decoder.json.decodeFromJsonElement(ListSerializer(ListSerializer(serializer<ShardId>())), jobj["Schedule"]!!))
          }
          if (jobj["Accounts"] != null) {
            return TrackedShardsConfig.Accounts(decoder.json.decodeFromJsonElement(ListSerializer(serializer<AccountId>()), jobj["Accounts"]!!))
          }
          if (jobj.size == 1) {
            ...
            when (key) {
              "Shards" -> {
                ...
                return decoder.json.decodeFromJsonElement(serializer<TrackedShardsConfig.Shards>(), obj)
              }
              "ShadowValidator" -> {
                val obj = valueElem as? JsonObject ?: throw SerializationException("Expected object payload for variant " + key)
                return decoder.json.decodeFromJsonElement(serializer<TrackedShardsConfig.ShadowValidator>(), obj)
              }
              "Schedule" -> {
                val obj = valueElem as? JsonObject ?: throw SerializationException("Expected object payload for variant " + key)
                return decoder.json.decodeFromJsonElement(serializer<TrackedShardsConfig.Schedule>(), obj)
              }
              "Accounts" -> {
                val obj = valueElem as? JsonObject ?: throw SerializationException("Expected object payload for variant " + key)
                return decoder.json.decodeFromJsonElement(serializer<TrackedShardsConfig.Accounts>(), obj)
              }
              "NoShards" -> {
