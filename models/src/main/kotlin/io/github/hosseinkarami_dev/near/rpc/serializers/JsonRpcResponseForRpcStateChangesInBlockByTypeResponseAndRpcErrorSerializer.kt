...
public object JsonRpcResponseForRpcStateChangesInBlockByTypeResponseAndRpcErrorSerializer : KSerializer<JsonRpcResponseForRpcStateChangesInBlockByTypeResponseAndRpcError> {
  ...
  override fun serialize(encoder: Encoder, `value`: JsonRpcResponseForRpcStateChangesInBlockByTypeResponseAndRpcError) {
    ...
    when (value) {
      is JsonRpcResponseForRpcStateChangesInBlockByTypeResponseAndRpcError.Result -> out.encodeSerializableElement(descriptor, 0, serializer<RpcStateChangesInBlockByTypeResponse>(), value.result)
      is JsonRpcResponseForRpcStateChangesInBlockByTypeResponseAndRpcError.Error -> out.encodeSerializableElement(descriptor, 1, serializer<RpcError>(), value.error)
    }
      ...
      when (element) {
        is JsonPrimitive -> {
          if (element.isString) {
            ...
            throw SerializationException("Unknown discriminator string for JsonRpcResponseForRpcStateChangesInBlockByTypeResponseAndRpcError: " + s)
          }
        }
        is JsonArray -> {
          ...
          if (jobj.size == 1) {
            ...
            when (key) {
              "result" -> {
                ...
                val resultVal = decoder.json.decodeFromJsonElement(serializer<RpcStateChangesInBlockByTypeResponse>(), obj["result"] ?: throw SerializationException("Missing field 'result' for variant " + key))
            ...
            val typeField = jobj["type"]?.jsonPrimitive?.contentOrNull ?: throw SerializationException("Missing 'type' discriminator in JsonRpcResponseForRpcStateChangesInBlockByTypeResponseAndRpcError")
            when (typeField) {
              "result" -> {
              ...
              else -> throw SerializationException("Unknown type discriminator for JsonRpcResponseForRpcStateChangesInBlockByTypeResponseAndRpcError: " + typeField)
            }
          }
        }
      }
