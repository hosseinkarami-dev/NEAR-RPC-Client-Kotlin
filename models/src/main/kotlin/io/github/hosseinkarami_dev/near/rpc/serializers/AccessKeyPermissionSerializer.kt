      ...
      when (element) {
        ...
        is JsonObject -> {
          ...
          if (jobj.size == 1) {
            ...
            val typeField = jobj["type"]?.jsonPrimitive?.contentOrNull ?: throw SerializationException("Missing 'type' discriminator in AccessKeyPermission")
            when (typeField) {
              "FunctionCall" -> {
              ...
              else -> throw SerializationException("Unknown type discriminator for AccessKeyPermission: " + typeField)
            }
          }
        }
      }
