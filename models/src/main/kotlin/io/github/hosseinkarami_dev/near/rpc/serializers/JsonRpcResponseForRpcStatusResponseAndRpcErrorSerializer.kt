          ...
          if (jobj.size == 1) {
            ...
            when (key) {
              "result" -> {
                ...
                return decoder.json.decodeFromJsonElement(serializer<JsonRpcResponseForRpcStatusResponseAndRpcError.Result>(), obj)
              }
              "error" -> {
                val obj = valueElem as? JsonObject ?: throw SerializationException("Expected object payload for variant " + key)
                return decoder.json.decodeFromJsonElement(serializer<JsonRpcResponseForRpcStatusResponseAndRpcError.Error>(), obj)
              }
