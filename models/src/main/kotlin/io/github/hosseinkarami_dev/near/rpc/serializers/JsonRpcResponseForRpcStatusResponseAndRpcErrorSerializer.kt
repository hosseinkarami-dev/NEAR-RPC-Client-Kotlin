...
public object JsonRpcResponseForRpcStatusResponseAndRpcErrorSerializer : KSerializer<JsonRpcResponseForRpcStatusResponseAndRpcError> {
  ...
  override fun serialize(encoder: Encoder, `value`: JsonRpcResponseForRpcStatusResponseAndRpcError) {
    ...
    when (value) {
      is JsonRpcResponseForRpcStatusResponseAndRpcError.Result -> out.encodeSerializableElement(descriptor, 0, serializer<RpcStatusResponse>(), value.result)
      is JsonRpcResponseForRpcStatusResponseAndRpcError.Error -> out.encodeSerializableElement(descriptor, 1, serializer<RpcError>(), value.error)
    }
      ...
      when (element) {
        ...
        is JsonObject -> {
          ...
          if (jobj.size == 1) {
            ...
            val typeField = jobj["type"]?.jsonPrimitive?.contentOrNull ?: throw SerializationException("Missing 'type' discriminator in JsonRpcResponseForRpcStatusResponseAndRpcError")
            when (typeField) {
              "result" -> {
              ...
              else -> throw SerializationException("Unknown type discriminator for JsonRpcResponseForRpcStatusResponseAndRpcError: " + typeField)
            }
          }
        }
      }
